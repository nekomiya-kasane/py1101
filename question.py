...
class Question:
    ...
    def copy_question(self):
        new_question = Question(self.qtype, self.description,
                                self.answer_options, self.correct_answer, self.marks)
        return new_question
    ...
    def set_answer_options(self, opts):
        ...
        if self.correct_answer != None:
            ...
            while i < len(opts):
                if self.qtype == "single":
                    new.append((opts[i][0], opts[i][0][0]
                               == self.correct_answer))
                if self.qtype == "multiple":
    ...
    def shuffle_answers(self):
        ...
        new_opt = []
        while i < 4:
            new_opt.append((letters[i] + old_opt[order[i]][0][1:],
                    old_opt[order[i]][1]))
            i += 1
        ...
        if self.qtype == "single":
            ...
            new_correct_answers = [
                letters[order[letters.index(ans)]] for ans in correct_answers]
